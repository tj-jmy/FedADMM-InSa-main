...
class Clients(object):
    ...
    def __init__(self, cfg, server_model, dataset):
        ...
        self._init_params(cfg, server_model)  # initialize parameters# 调用内部方法初始化参数，包括拉格朗日乘子和惩罚参数
    ...
    def _stop_check(self, count):
        ...
        elif self.cfg.alg in ["admm_insa", "admm_in"]: # 检查 ADMM 的不精确准则，仅在特定算法下启用
    ...
    def _local_step_personalize(self, cfg):
        ...
        if cfg.alg in ["admm_insa", "admm_in", "admm"]:
            ...
            self._update_lambda(beta_i_k)  # update dual variables lambda  # 更新拉格朗日乘子 lambda
            if self.cfg.alg == "admm_insa":  # udpate penlaty parameter beta 如果是 admm_insa（自适应 ADMM），更新惩罚参数 beta
    ...
    def _update_lambda(self, beta):
        ...
        for name, _ in self.model_i.named_parameters():  # without BN statistics
            ...
            self.lamda[self.i][name] += beta * (u_state[name] - z_state[name]) # 根据文章1的拉格朗日乘子更新公式，更新逻辑应改为加法而非减法。
    def _adp_beta(self, beta):
        ...
        primal_residual = beta * l2_norm(u_kplus1, u_k) # 原始残差，用于度量本地模型和上次更新间的差异，调整约束强度。
    ...
    def _inexact_check(self, count):
        ...
        if count == 0:
            self.e_u = self._inexact_e(self.model_server)  # use z^k instead of u_i^k# 使用全局模型参数 z^k
        # 计算新的不精确度 e_u
    ...
    def _grad(self, model, data, labels):
        ...
        pred = model(data)
        # 计算损失
        loss = model.loss(pred, labels)  # default reduction == "mean"# 默认使用 "mean" 作为损失的归约方式
    ...
    def _admm_u(self, model, data, labels):
        ...
        for name, param in model.named_parameters():  # without BN statistics# 忽略 BN 统计量
            ...
            # param.grad += self.beta[self.i][self.k] * (u_state[name] - z_state[name])
            param.grad += self.beta[self.i][self.k] * (u_state[name] - z_state[name]) * self.alpha[self.i]
    ...
    def _init_params(self, cfg, model_server):
        ...
        self.steps = np.full((cfg.m, cfg.K + 1), 0)  # to save local FGD/SGD steps# 保存本地FGD/SGD步数
        self.accus = np.full((cfg.m, cfg.K + 1), 0, dtype=float)  # local model accuracy# 保存本地模型准确率
        ...
        if cfg.alg in ["admm_insa", "admm_in", "admm"]:
            ...
            self.lamda = [copy.deepcopy(lamda) for _ in range(cfg.m)]  # 为每个客户端复制拉格朗日乘子
